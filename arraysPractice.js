...
const divider = (numbersArray) => {
  let evens = numbersArray.filter((x)=>x%2===0);
  let odds = numbersArray.filter((y)=>y%2!==0);
  return divided = [evens,odds];
}
...
const addTen = (numbers) => {
  let newNumbers = numbers.map((x)=>parseInt(x)+10);
  return newNumbers;
}
...
for (let i=0;i<devMountainEmployees.length;i++) {
  if (cahlan===devMountainEmployees[i]) {
...
users = [
  user1, 
  {name: 'Jin', email: 'abc@gmail.com', password: 'aswer', username: 'Cool'},
  {name: 'Bob', email: 'bcg@yahoo.com', password: 'asdf', username: 'Lame'}
];
